...
// === WiFi Config ===
const char* sta_ssid     = "StimStep";  
const char* sta_password = "password";
const char* backendURL   = "http://www.stimstep.sphinxsecrets.com/api/emg-data";

WebServer server(80);

// === FES parameters ===
uint8_t pulse_frequency = 30;  // 30 Hz gives smooth, fatigue-resistant contractions
unsigned long train_duration_ms = 400;  // 400 ms matches average swing phase
...
unsigned long cooldown_time = 3600000;  // 60 minutes
...
void loop() {
  ...
  if (fabs(rate) > detect.threshold) {
     ...
     else if (session_time >= MAX_SESSION_TIME) 
     {
        // Start cooldown
         if (!need_cooldown) 
         {
      ...
      else if (need_cooldown) 
      {
        unsigned long cooldown_passed = millis() - cooldown_start;

        if (cooldown_passed >= cooldown_time)
        {
            // Cooldown finished - reset everything
            need_cooldown = false;
            session_start_time = millis();  // Reset session timer
            can_stimulate = true;
            Serial.println("Cooldown finished - ready to stimulate again");
        }
        ...
        else 
        {
            ...
            Serial.print((cooldown_time - cooldown_passed) / 60000);
      ...
      // === Check WiFi === 
      // If WiFi just disconnected, save data
      if (WiFi.status() != WL_CONNECTED && wifi_was_connected) {
          Serial.println("WiFi lost - saving data");
          save_locally();
          wifi_was_connected = false;
      }
      
      // If WiFi just reconnected, upload saved data
      if (WiFi.status() == WL_CONNECTED && !wifi_was_connected) {
          Serial.println("WiFi back - uploading saved data");
          upload_local_data();
      ...
      if (can_stimulate) {
          ...
          begin_emg_sampling(train_duration_ms + 100);
  ...
  if (millis() - last_status > 60000) {  // 60 seconds
      last_status = millis();
      show_status();
  }
...
// === Save data locally in case wifi disconnects ===
void save_locally() {
  ...
  if (file) {
    ...
    emg_sample_count = 0;  // Clear buffer after saving
...
// === Upload locally saved data when wifi reconnects ===
void upload_local_data() {
  ...
  if (!file || file.size() == 0) {
    ...
    if (file.size() == 0) {
    Serial.println("Backup file is empty");
    file.close();
    return;
    }
